...
def get_answer(first, diff_ratio, terms, apgp, sum_terms,comp):#Function decides the answer based on AP or GP
    try:
        if(apgp==0 and sum_terms==0):
            return first+(terms-1)*diff_ratio
        elif(apgp==0 and sum_terms==1):
            return terms//2 * (2*first + (terms-1)*diff_ratio)
        elif(apgp==1 and sum_terms==0):
            return first*(diff_ratio**(terms-1))
        else:
            return first*((diff_ratio**terms)-1)//(diff_ratio-1)
    except:
        get_complexity(comp)
        exit(1)

def get_question(first, diff_ratio, terms, comp, apgp ,sum_terms):#Function generates the random question of AP or GP
    q = get_q(first, diff_ratio, apgp)
    answer = get_answer(first, diff_ratio, terms, apgp, sum_terms,comp)
    if(answer>10000 or answer<(-10000)):
        get_complexity(comp)
        exit(1)
    d = [-2, 1, -1, 2]
    if(comp == 2):
    ...
    genarate_question(terms, apgp, sum_terms,options, answer,q)

def genarate_question(terms, apgp, sum_terms,options, answer,q):
    ch = 'A.P' if(apgp == 0) else 'G.P'
    if(sum_terms==0):
        question = "Find the "+str(terms)+"th term of the " + ch + " series, "
    else:
        question ="Find the sum of first "+str(terms)+" terms of the "+ch+" series, "
...
def get_terms(comp, boundary, apgp, sum_terms):#Function Gets the terms based on complexity
    if(comp == 1):
        ...
        first = round(rand.uniform(boundary[comp][0][0], boundary[comp][0][1]), 4)
        diff_ratio = round(rand.uniform(boundary[comp][1][0], boundary[comp][1][1]), 4)
        ...
        terms = zero_check(rand.randint(boundary[comp][2][0], boundary[comp][2][1]), 3)
    get_question(first, diff_ratio, terms, comp, apgp, sum_terms)
...
def get_complexity(comp):#Function decides the boundary based on the complexity and (AP or GP)
    ...
    boundary2 = {
        1: ((-101, 100), (-10, 10), (4, 7)),
        2: ((-101.5, 100.5), (-5, 5), (4, 7)),
        3: ((-10, 10), (-5, 5), (4, 7))
    }
    if(apgp == 0):
        get_terms(comp, boundary1, apgp, sum_terms)
    else:
        get_terms(comp, boundary2, apgp, sum_terms)
